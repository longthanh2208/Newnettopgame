...
MAIL_LIMIT = 5
MAIL_BLOCK_DURATION = 300  # 5 phút (tính bằng giây)
...
def forgot_password():
    ...
    # Lọc bỏ các lượt gửi cũ hơn 5 phút
    ...
    if len(recent_attempts) >= MAIL_LIMIT:
        ...
        return jsonify({
            "status": "error", 
            "message": f"Bạn đã gửi quá nhanh! Vui lòng thử lại sau {wait_time} giây."
        }), 429
        
    reset_token = secrets.token_urlsafe(32)
    user['reset_token'] = reset_token
    user['reset_token_expiry'] = now + 900 # 15 phút
    
    # Cập nhật danh sách gửi mail
...
def add_review():
    ...
    if not order or order.get('status') != "Đã giao":
        return jsonify({"status": "error", "message": "Bạn chỉ có thể đánh giá sau khi nhận hàng!"}), 403
        
    reviews = load_data(REVIEW_DB_PATH)
    if any(r.get('order_id') == order_id and r.get('product_id') == product_id for r in reviews):
        return jsonify({"status": "error", "message": "Bạn đã đánh giá sản phẩm này rồi!"}), 400
    ...
    return jsonify({"status": "success", "message": "Cảm ơn bạn đã đánh giá!"})

@app.route("/api/reviews/<int:p_id>", methods=["GET"])
def get_product_reviews(p_id):
    reviews = load_data(REVIEW_DB_PATH)
    product_reviews = [r for r in reviews if int(r['product_id']) == p_id]
